...
import { resolveRolePermissions, hasPermission } from '../auth/permissions';
...
export class OrganizationService {
  ...
  async updateLeaveApproval(orgId: string, dto: UpdateLeaveApprovalDto) {
    ...
    if (dto.leaveHrApproverUserId) {
      ...
      if (!hasPermission(resolveRolePermissions(u.role?.permissions), 'leave.approve')) {
        throw new BadRequestException('The leave approver must hold a role with leave.approve permission');
      }
    }
